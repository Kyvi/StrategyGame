...
public class ChangeNourriture : MonoBehaviour {
	...
	public void change(){
		...
		if ( intInput <= gv.minionDisponible + gv.minionnourriture) {
			gv.minionDisponible = gv.minionDisponible + gv.minionnourriture - intInput;
			gv.minionnourriture = intInput;
			...
			while (compteur > 0 && k<gv.minions.Count) {
				if ( ((SoldatMinion)gv.minions[k]).metier == 0 ){
					((SoldatMinion)gv.minions[k]).metier = 3;
					compteur--;
				}
				k++;
			}
