...
public class ChangeBois : MonoBehaviour {
	...
	public void change(){
		...
		if ( intInput <= gv.minionDisponible + gv.minionbois) {
			gv.minionDisponible = gv.minionDisponible + gv.minionbois - intInput;
			gv.minionbois = intInput;
			...
			while (compteur > 0 && k<gv.minions.Count) {
				if ( ((SoldatMinion)gv.minions[k]).metier == 0 ){
					((SoldatMinion)gv.minions[k]).metier = 1;
					compteur--;
				}
				k++;
			}
